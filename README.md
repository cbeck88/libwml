...
- A map of *config children*, like this:
  ...
  ```
  which can represent all of the subtags of a given tag, and keep track of the order of similarly-named tags. (This isn't quite accurate but it doesn't matter right now.)
...
The `schemas/` directory contains a `.xml` file describing the list of all known WML tags.

The `py/` directory contains a python script which reads the xml and emits a C++ header, with structures corresponding to all the tags in the spec.
...
in `lib/` if you don't have that already. You also need the header file generated by the python script, if you want to make use of wesnoth ast information.
